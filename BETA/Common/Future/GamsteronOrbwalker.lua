...
--[------------------------------------------------------------------------------------------------------------------------------------------------------------]]
        ...
        local function GetLaneMinion_GSO(tEnemyM, HeroAD_GSO)
                local result      = nil
                local LaneMTable  = {}
                local tAllyM      = GetAllyMinions_GSO(2000)
                for i = 1, #tEnemyM do
                        local EnemyM        = t[i]
                        local PossibleDmgA  = 0
                        local Count         = 0
                        for i = 1, #tAllyM do
                                local AllyM = t[i]
                                if math.sqrt((EnemyM.pos.x-AllyM.pos.x)^2 + (EnemyM.pos.z-AllyM.pos.z)^2) < AllyM.range + AllyM.boundingRadius + EnemyM.boundingRadius + 50 then
                                        PossibleDmgA = PossibleDmgA + AllyM.totalDamage
                                end
                                Count = Count + 1
                        end
                        if Count == 0 or EnemyM.health - PossibleDmgA > (LocalBonusDmgUnit_GSO(EnemyM) + HeroAD_GSO) * 1.5 then
        ...
        local function GetTarget_GSO(combo, lane, harass, lasthit)
                ...
                elseif lane then
                        ...
                        if lasthitsoon[1] == nil and lasthitsoon[3] == false then
                                AAtarget = GetLaneMinion_GSO(t, HeroAD_GSO)
                        else
